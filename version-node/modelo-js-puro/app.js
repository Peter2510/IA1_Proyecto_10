...
const loadModel = async () => {
  try {
    ...
    const model = await tf.loadLayersModel('http://127.0.0.1:5500/modelo_chatbot/my-model.json');
    //console.log(model)
...
// Función para predecir la respuesta
const predictResponse = async (inputText, model, maxInputLength) => {
  // Tokenización de la entrada
  const tokenizedInput = tokenize(inputText); 

  // Asegúrate de que la longitud sea la correcta, rellenando con ceros si es necesario
  const paddedInput = [...tokenizedInput];
  while (paddedInput.length < maxInputLength) {
    paddedInput.push(0);  // Rellenar con ceros
  }
  
  // Limitar la longitud para no exceder maxInputLength
  const inputTensor = tf.tensor2d([paddedInput.slice(0, maxInputLength)]).toFloat();
...
const prueba = async () => {
 await trainModel();  // Entrenar el modelo primero
  const model = await loadModel();  // Cargar el modelo

  // Entrenamiento y carga del modelo exitosos, ahora realizar una predicción
  ...
  predictResponse(userInput, model, 9);
};
